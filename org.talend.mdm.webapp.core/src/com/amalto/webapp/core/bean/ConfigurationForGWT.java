...
import com.amalto.webapp.core.gwt.GwtWebContext;
...
public class ConfigurationForGWT {
    ...
    private static final Logger LOG = Logger.getLogger(ConfigurationForGWT.class);
    ...
    public static ConfigurationForGWT getInstance(boolean forceReload) throws Exception {
        ConfigurationForGWT instance;
        if (forceReload) {
    ...
    public static ConfigurationForGWT getInstance() throws Exception {
        ConfigurationForGWT instance;
        HttpSession session = getSessionFromContext();
        if (session == null) {
            instance = null;
        } else {
            instance = (ConfigurationForGWT) session.getAttribute(CONFIGURATION_ATTRIBUTE);
        }
        if (instance == null) {
            if (LOG.isDebugEnabled())
                LOG.debug("ConfigurationForGWT instance is null, loading ..."); //$NON-NLS-1$
            instance = load(session);
        }
        return instance;
    }

    public static ConfigurationForGWT getConfiguration() throws Exception {
        return getInstance();
    }
    ...
    public static void initialize(String cluster, String model) throws Exception {
        ...
        if (session != null)
            session.setAttribute(CONFIGURATION_ATTRIBUTE, new ConfigurationForGWT(cluster, model));
    }
    ...
    private static ConfigurationForGWT load(HttpSession session) throws Exception {
        ConfigurationForGWT configuration = loadConfigurationFromDB();
        if (session != null)
    ...
    public static ConfigurationForGWT loadConfigurationFromDBDirectly() throws Exception {
        return loadConfigurationFromDB();
    }

    private static synchronized ConfigurationForGWT loadConfigurationFromDB() throws PolicyContextException, Exception,
            XtentisWebappException {
        ConfigurationForGWT configuration = new ConfigurationForGWT();

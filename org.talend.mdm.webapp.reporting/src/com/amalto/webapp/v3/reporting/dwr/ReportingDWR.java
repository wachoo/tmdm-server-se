...
public class ReportingDWR {
    ...
    public Map<String, String> getReportingsName(String value) throws XtentisWebappException, Exception {
        ...
        WSWhereCondition wc = new WSWhereCondition("Reporting/Owner", WSWhereOperator.EQUALS,
                Util.getAjaxSubject().getUsername(), WSStringPredicate.OR, false);
        WSWhereCondition wc2 = new WSWhereCondition("Reporting/Shared", WSWhereOperator.EQUALS, "true", WSStringPredicate.OR,
                false);
        /*
         * ArrayList<WSWhereItem> conditions=new ArrayList<WSWhereItem>(); conditions.add(new
         * WSWhereItem(wc,null,null)); conditions.add(new WSWhereItem(wc2,null,null)); WSWhereOr or=new
         * WSWhereOr((WSWhereItem[])conditions.toArray(new WSWhereItem[conditions.size()]));
         */
        ...
        String[] results = Util.getPort().xPathsSearch(new WSXPathsSearch(new WSDataClusterPK(_reportingCluster), null,// pivot
                new WSStringArray(new String[] { "Reporting/ReportingName" }), wi, -1, 0, Integer.MAX_VALUE, null, // order
                                                                                                                   // by
                null // direction
                )).getStrings();
        ...
        for (int i = 0; i < results.length; i++) {
            results[i] = results[i].replaceAll(
                    "<result>\\r*\\n*\\s*<ReportingName(.*?)>(.*)</ReportingName>\\r*\\n*\\s*</result>\\r*\\n*\\s*", "$2"); //$NON-NLS-1$ //$NON-NLS-2$;
    ...
    public Reporting getReporting(String reportingName) throws XtentisWebappException, Exception {
        ...
        String result = Util
                .getPort()
                .getItem(
                        new WSGetItem(new WSItemPK(new WSDataClusterPK(_reportingCluster), "Reporting",
                                new String[] { reportingName }))).getContent();
    ...
    public ArrayList<ReportingContent> getReportingContent(String reportingName, String[] parameters)
            throws XtentisWebappException, Exception {

        return (ArrayList<ReportingContent>) getReportingContent(reportingName, parameters, 0, Integer.MAX_VALUE, "0", "ASC")
                .get(0);

    }

    // TODO configuration
    public ArrayList getReportingContent(String reportingName, String[] parameters, int skip, int max, String sortCol,
            String sortDir) throws XtentisWebappException, Exception {
        ...
        if (reporting.getStoredProcedure() != null && !reporting.getStoredProcedure().equals("")) {
            ...
            org.apache.log4j.Logger.getLogger(this.getClass()).debug(
                    "getReportingContent - execute storedProcedure " + reporting.getStoredProcedure());
            // execute query, replacing $parameters with values coming from input fields
            String[] results = Util
                    .getPort()
                    .executeStoredProcedure(
                            new WSExecuteStoredProcedure(new WSStoredProcedurePK(reporting.getStoredProcedure()), null,
                                    new WSDataClusterPK(reporting.getCluster()), parameters)).getStrings();

            org.apache.log4j.Logger.getLogger(this.getClass()).debug(
                    "getReportingContent - execute storedProcedure " + reporting.getStoredProcedure() + " nb result = "
                            + results.length);
            ...
            for (int i = 0; i < results.length; i++) {
                ...
                org.apache.log4j.Logger.getLogger(this.getClass()).trace(
                        "getReportingContent - result[" + i + "] = " + results[i]);
            ...
            if (filterXpaths != null && filterValues != null)
                for (int i = 0; i < filterValues.length; i++) {
                    if ((filterValues[i] == null) || (filterValues[i] == "*") || "".equals(filterValues[i]))
                        continue;
                    ...
                    WSWhereCondition wc = new WSWhereCondition(filterXpaths[i], getOperator(filterOperators[i]), filterValues[i],
                            WSStringPredicate.NONE, false);
                    // System.out.println("iterator :"+i+"field - getErrors- : " + fields[i] + " " + operator[i]);
                    // System.out.println("Xpath field - getErrors- : " + giveXpath(fields[i]) + " - values : "+
                    // regexs[i]);
            ...
            if (sortCol != null)
                sortColumn = reporting.getFields()[Integer.parseInt(sortCol)].getXpath();
            ...
            String[] results = Util.getPort()
                    .xPathsSearch(new WSXPathsSearch(new WSDataClusterPK(reporting.getCluster()), reporting.getPivotXpath(),// pivot
                            new WSStringArray(xpaths), wi, 0, skip, max, sortColumn, // order by
                            sortDirection // direction
                            )).getStrings();

            WSString wsString = Util.getPort().count(
                    new WSCount(new WSDataClusterPK(reporting.getCluster()), reporting.getPivotXpath(), wi, -1));
    ...
    public Map<String, String> getStoredProcedurePKs() {
        ...
        try {
            WSStoredProcedurePK[] pkArray = Util.getPort().getStoredProcedurePKs(new WSRegexStoredProcedure(".*"))
                    .getWsStoredProcedurePK();
    ...
    public Map<String, String> getXpathToLabel(String dataObject, String language) {
        ...
        try {
            Configuration config = Configuration.getInstance();
            String dataModelPK = config.getModel();
            xpathToLabel = CommonDWR.getFieldsByDataModel(dataModelPK, dataObject, language, false);
    ...
    /*
     * public String[] getElements(String dataObject,String language){ try { LinkedHashMap<String, String> sortedMap =
     * getXpathToLabel(dataObject, language);
     */
    /*
     * TreeSet<Map.Entry> set = new TreeSet<Map.Entry>(new Comparator() { public int compare(Object obj, Object obj1) {
     * return ((Comparable) ((Map.Entry) obj).getValue()) .compareTo(((Map.Entry) obj1).getValue()); } });
     * set.addAll(xpathToLabel.entrySet()); LinkedHashMap<String, String> sortedMap = new LinkedHashMap<String,
     * String>(); for (Iterator i = set.iterator(); i.hasNext();) { Map.Entry entry = (Map.Entry) i.next();
     * sortedMap.put((String) entry.getKey(), (String) entry.getValue()); }
     */
    /*
     * Collection<String> list = sortedMap.keySet(); String[] elements = (String[])(list.toArray(new
     * String[list.size()]));
     * 
     * return elements; } catch (Exception e) { e.printStackTrace(); return null; }
     * 
     * }
     */
    public void deleteReporting(String reportingName) throws XtentisWebappException, Exception {
        if (getReporting(reportingName).isErasable() == true
                || (getReporting(reportingName).isErasable() == false && getReporting(reportingName).isUnErasableButDeleteable() == true)) {
            Util.getPort().deleteItem(
                    new WSDeleteItem(new WSItemPK(new WSDataClusterPK(_reportingCluster), "Reporting",
                            new String[] { reportingName })));
        }
    }

    public String saveReporting(Reporting reporting, String concept, String language, boolean edit)
            throws XtentisWebappException, Exception {
        if (!"".equals(reporting.getReportingName())) {
            Configuration config = Configuration.getInstance(true);
            // HashMap<String,String> fields = CommonDWR.getFieldsByDataModel(config.getModel(),concept, language,
            // false);
            WebContext ctx = WebContextFactory.get();
            ...
            if (edit == false || getReporting(reporting.getReportingName()).isErasable() == false) {
                boolean itemExist = Util
                        .getPort()
                        .existsItem(
                                new WSExistsItem(new WSItemPK(new WSDataClusterPK(_reportingCluster), "Reporting",
                                        new String[] { reporting.getReportingName() }))).is_true();
            ...
            Util.getPort().putItem(
                    new WSPutItem(new WSDataClusterPK(_reportingCluster), reporting.serialize(), new WSDataModelPK(
                            _reportingModel), false));
     ...
     * @return
     */
    public int getNumberOfParamters(String name) throws XtentisWebappException, Exception {
        String proceduce = "";

        try {
            WSGetStoredProcedure storedProcedure = new WSGetStoredProcedure(new WSStoredProcedurePK(name));

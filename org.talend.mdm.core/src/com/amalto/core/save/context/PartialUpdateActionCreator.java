            ...
            if (!newAccessor.exist()) {
                ...
                if (newAccessor.get() != null && !newAccessor.get().isEmpty()) { // Empty accessor means no op to ensure
                    ...
                    if (comparedField.isMany() && preserveCollectionOldValues) {
                        Accessor leftAccessor = originalDocument.createAccessor(leftPath.substring(0, leftPath.lastIndexOf("["))); //$NON-NLS-1$
                        ...
                        this.leftPath.push(comparedField.getName() + "[" + (newIndex + 1) + "]"); //$NON-NLS-1$ //$NON-NLS-2$

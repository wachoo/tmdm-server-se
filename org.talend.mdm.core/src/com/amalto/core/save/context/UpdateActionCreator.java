...
class UpdateActionCreator extends DefaultMetadataVisitor<List<Action>> {
    ...
    public UpdateActionCreator(MutableDocument originalDocument, MutableDocument newDocument, Date date,
            boolean preserveCollectionOldValues, int insertIndex, String source, String userName, MetadataRepository repository) {
            ...
            if (!newAccessor.exist()) {
                ...
                if (newAccessor.get() != null && !(comparedField instanceof ContainedTypeFieldMetadata)) {
                    if (comparedField.isMany() && preserveCollectionOldValues) {
                        ...
                        this.path.push(comparedField.getName() + "[" + insertIndex + "]"); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (rightAccessor.exist()) {
                ...
                if (!newType.isEmpty()) {
                    ...
                    if (!newTypeMetadata.getSuperTypes().isEmpty() || !newTypeMetadata.getSubTypes().isEmpty()) {
                        actions.add(new ChangeTypeAction(date, source, userName, getLeftPath(), previousTypeMetadata,
                                newTypeMetadata));
            ...
            if (field instanceof ReferenceFieldMetadata) {
                ...
                if (rightAccessor.exist()) {
                    ...
                    if (!newType.isEmpty()) {
                        ...
                        if (!newTypeMetadata.getSuperTypes().isEmpty() || !newTypeMetadata.getSubTypes().isEmpty()) {
                            actions.add(new ChangeReferenceTypeAction(date, source, userName, getLeftPath(),
                                    previousTypeMetadata, newTypeMetadata));
                        } else if (LOGGER.isDebugEnabled()) {

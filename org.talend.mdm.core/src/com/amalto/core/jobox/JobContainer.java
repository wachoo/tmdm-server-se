...
public class JobContainer {
	...
	public void init(Properties props) {
		...
		if(!joboxDeployPath.exists())joboxDeployPath.mkdirs();
		if(!joboxWorkPath.exists())joboxWorkPath.mkdirs();	
		org.apache.log4j.Logger.getLogger(this.getClass()).info("Jobox Home is: "+joboxConfig.getJoboxHome());
		//init component
		...
		for (Iterator iterator = currentJobs.iterator(); iterator.hasNext();) {
			...
			if(!this.jobLoadersPool.containsKey(jobInfo))this.jobLoadersPool.put(jobInfo, cl);
		}
		//start monitor
		// Create the monitor
	...
	public void updateJobLoadersPool(JobInfo jobInfo) {
		...
		if(this.jobLoadersPool.containsKey(jobInfo)) {
			...
			org.apache.log4j.Logger.getLogger(this.getClass()).info("Removing "+jobClassLoader);
			jobClassLoader=null;

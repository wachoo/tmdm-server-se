...
public class StagingAreaTest extends TestCase {
    ...
    private void generateData(boolean validData) {
        ...
        allRecords.clear();
        allRecords.add(factory.read("MDM", 1, person, newPerson(0, validData)));
        try {
            origin.begin();
        ...
        for (int i = 0; i < COUNT; i++) {
            allRecords.add(factory.read("MDM", 1, country, newCountry(i, validData)));
        }
        ...
        allRecords.clear();
        for (int i = 0; i < COUNT; i++) {
            allRecords.add(factory.read("MDM", 1, address, newAddress(i, i % 2 != 0, validData)));
        }
        ...
        allRecords.clear();
        for (int i = 0; i < COUNT; i++) {
            allRecords.add(factory.read("MDM", 1, person, newPerson(i, validData)));
        }
    ...
    private class TestCommitter implements SaverSession.Committer {
        ...
        public void save(ItemPOJO item, String revisionId) {
            try {
                ...
                DataRecord dataRecord = reader.read(storage.getName(), 1, complexType, item.getProjection());

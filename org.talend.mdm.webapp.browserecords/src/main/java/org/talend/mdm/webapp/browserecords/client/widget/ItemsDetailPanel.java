...
public class ItemsDetailPanel extends ContentPanel {
    ...
    private void initPanel() {
        ...
        mainPanel.addListener(Events.Resize, 
    ...
    public void initBreadCrumb(BreadCrumb breadCrumb) {
    	breadCrumb.getElement().setId("ItemsDetailPanel-breadCrumb");
    ...
    /**
     * Custom tab panel within ItemsDetailPanel that supports the desired
    ...
    private class ItemsDetailTabPanel extends LayoutContainer
    {
		...
		/**
		 * Called by resize listener to recalculate what tabContent and 
		 * the within it should be in size. What is within must be smaller
		 * due to borders of tabContent.
		 */
        private void calcTabContentSize()
        {
        	this.curTabContentHeight = this.getHeight() - TAB_BAR_HEIGHT - TAB_BAR_CONTENT_DIVIDER_HEIGHT;
        	if (this.curTabContentHeight < 0) this.curTabContentHeight = 0;

        	this.curTabContentWidth = this.getWidth();
        	
        	this.curTabContentInnerHeight = this.curTabContentHeight - 1;
        	if (this.curTabContentInnerHeight < 0) this.curTabContentInnerHeight = 0;
        	
        	this.curTabContentInnerWidth = this.curTabContentWidth - 2;
        	if (this.curTabContentInnerWidth < 0) this.curTabContentInnerWidth = 0;
        }
        ...
        /**
         * This does nothing if index is not valid. If tab is selected, tries
         * to select next one if there is one, and if not, select the previous
         * one.
         * 
        ...
        public void closeTabAtIndex(int index)
        {
    		...
    		if (index >= 0 && index < tabCount) {
    			...
    			else if (index == this.tabBar.getSelectedTab()) 
    			{// Attempting to close selected tab and at least one more remains
    	...
    	/**
    	 * Returns widget contained in currently selected tab.
    	 * Returns null if no currently selected tab.
    	 * 
    	...
    	/**
    	 * Returns ItemPanel in currently selected tab. If no currently
    	 * selected tab or widget not ItemPanel, null is returned.
    	 * 
    	...
    	/**
    	 * Get widget in tab at specified index. Returns null if
    	 * index is not valid.
    	 * 
    	 * @param index
    	 * @return
    ...
    /**
     * Initialize the gray outer borders of the ItemsDetailPanel using
     * a border layout to emulate the HTML technique of using an image
    ...
    {
    	BorderLayoutData result = new BorderLayoutData(region,
    			width, width, width);

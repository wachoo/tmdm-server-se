...
public class DownloadData extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<String> headerList = org.talend.mdm.webapp.base.shared.util.CommonUtil.convertStrigToList(new String(request
                .getParameter("header").getBytes("iso-8859-1"), "UTF-8"), Constants.FILE_EXPORT_IMPORT_SEPARATOR); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    private void getTableContent(HSSFSheet sheet, String viewPK, String concept, boolean includeXmlContent,
        ...
        List<String> viewableXpathList = org.talend.mdm.webapp.base.shared.util.CommonUtil.convertStrigToList(
                request.getParameter("viewableXpath"), Constants.FILE_EXPORT_IMPORT_SEPARATOR); //$NON-NLS-1$
        Map<String, EntityModel> foreignKeyEntityMap = org.talend.mdm.webapp.browserecords.server.util.CommonUtil
                .getForeignKeyEntityMap(entity, viewableXpathList, language);

        String[] results = null;
        if (request.getParameter("itemXmlString") != null && !request.getParameter("itemXmlString").isEmpty()) { //$NON-NLS-1$ //$NON-NLS-2$
            List<String> resultlList = org.talend.mdm.webapp.base.shared.util.CommonUtil.convertStrigToList(
                    request.getParameter("itemXmlString"), Constants.FILE_EXPORT_IMPORT_SEPARATOR); //$NON-NLS-1$
            results = resultlList.toArray(new String[resultlList.size()]);
        } else {
            results = CommonUtil
                    .getPort()
                    .viewSearch(
                            new WSViewSearch(new WSDataClusterPK(dataCluster), new WSViewPK(viewPK),
                                    (criteria != null ? CommonUtil.buildWhereItem(criteria) : null), -1, new Integer(0),
                                    maxCount, sortField, sortDir)).getStrings();
        }
        ...
        for (int i = 1; i < results.length; i++) {
            ...
            org.talend.mdm.webapp.browserecords.server.util.CommonUtil.dynamicAssembleByResultOrder(itemBean, viewableXpathList,
                    entity, foreignKeyEntityMap, language);
            if (includeXmlContent) {
                StringBuilder ids = new StringBuilder();
                for (String key : entity.getKeys()) {
                    org.w3c.dom.Document document = org.talend.mdm.webapp.browserecords.server.util.CommonUtil
                            .parseResultDocument(results[i], "result"); //$NON-NLS-1$
                    ids = ids.length() != 0 ? ids.append(".") : ids; //$NON-NLS-1$
                    ids = ids.append(Util.getFirstTextNode(document.getDocumentElement(),
                            "." + key.substring(key.lastIndexOf('/')))); //$NON-NLS-1$
                }
                WSItem wsItem = CommonUtil.getPort().getItem(
                        new WSGetItem(new WSItemPK(new WSDataClusterPK(dataCluster), concept, CommonUtil.extractIdWithDots(ids
                                .toString()))));

...
public class DataProviderTest extends TestCase {
    ...
    public void testGetDataResult() throws Exception {
        ...
        assertEquals("<result xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">" + "    <Id>1</Id>"
                + "    <Name>z@@zz@Z</Name>" + "    <Family>[1]</Family>" + "    <Price>111.00</Price>" + "    <Availability/>"
                + "</result>", new DataProvider("dataCluster", "Product", "viewPk", "criteria", 0, "sortDir", "sortField",
                "language", result).getDataResult()[1].replace("\r", "").replace("\n", ""));

        assertEquals("DataInDB1", new DataProvider("dataCluster", "Product", "viewPk", "criteria", 0, "sortDir", "sortField",
                "language", "").getDataResult()[1]);
    }

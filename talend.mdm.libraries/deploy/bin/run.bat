...
set JAVA_OPTS=%JAVA_OPTS% -Xms512m -Xmx1024m -XX:MaxPermSize=256m -Djboss.server.exitonshutdown=true -Djboss.shutdown.forceHalt=false -Dorg.exist.xmldb.inMemory.remote.content=true -Dorg.exist.xmlrpc.inMemory.retrieve.content=true -Djava.util.logging.config.file=%JBOSS_HOME%\server\default\conf\logging.properties

 ...
 * 
 * The contents of this file are subject to the terms of the following open source licenses: LGPL 3.0 or LGPL 2.1 or
 * CDDL 1.0 (the "Licenses"). You can select the license that you prefer but you may not use this file except in
 * compliance with one of these Licenses.
 * 
 * You can obtain a copy of the LGPL 3.0 license at http://www.gnu.org/licenses/lgpl-3.0.html
 * 
 * You can obtain a copy of the LGPL 2.1 license at http://www.gnu.org/licenses/lgpl-2.1.html
 * 
 * You can obtain a copy of the CDDL 1.0 license at http://www.sun.com/cddl/cddl.html
 * 
 * See the Licenses for the specific language governing permissions and limitations under the Licenses.
 * 
 * Alternatively, you can obtain a royaltee free commercial license with less limitations, transferable or
 * non-transferable, directly at http://www.noelios.com/products/restlet-engine
 * 
...
/**
 * Servlet acting like an HTTP server connector. See <a href="/documentation/1.1/faq#02">Developper FAQ #2</a> for
 * details on how to integrate a Restlet application into a servlet container.<br>
 * <br>
 * Initially designed to deploy a single Restlet Application, this Servlet can now deploy a complete Restlet Component.
 * This allows you to reuse an existing standalone Restlet Component, potentially containing several applications, and
 * declaring client connectors, for example for the CLAP, FILE or HTTP protocols.<br>
 * <br>
 * There are three separate ways to configure the deployment using this Servlet. They are described below by order of
 * priority:
 * <table>
 * <tr>
 * <th>Mode</th>
 ...
 * <tr>
 * <td><b>1</b></td>
 * <td>If a "/WEB-INF/restlet.xml" file exists and contains a valid XML configuration as described in the documentation
 * of the {@link Component} class. It is used to instantiate and attach the described component, contained applications
 * and connectors.</td>
 * </tr>
 * <tr>
 * <td><b>2</b></td>
 * <td>If the "/WEB-INF/web.xml" file contains a context parameter named "org.restlet.component", its value must be the
 * path of a class that inherits from {@link Component}. It is used to instantiate and attach the described component,
 * contained applications and connectors.</td>
 * </tr>
 * <tr>
 * <td><b>3</b></td>
 * <td>If the "/WEB-INF/web.xml" file contains a context parameter named "org.restlet.application", its value must be
 * the path of a class that inherits from {@link Application}. It is used to instantiate the application and to attach
 * it to a default Restlet Component.</td>
 * </tr>
 * </table>
 * <br>
 * In deployment mode 3, you can also add an optionnal "org.restlet.clients" context parameter that contains a space
 * separated list of client protocols supported by the underlying component. For each one, a new client connector is
 * added to the Component instance.<br>
 * 
 ...
 * </pre>
 * 
 * Note that the enumeration of "initParameters" of your Servlet will be copied to the "context.parameters" property of
 * your Restlet Application. This way, you can pass additional initialization parameters to your application, and maybe
 * share them with other Servlets.<br>
 * <br>
 * An additionnal boolean parameter called "org.restlet.autoWire" allows you to control the way your customized
 * Component fits in the context of the wrapping Servlet. The root cause is that both your Servlet Container and your
 * Restlet Component handle part of the URI routing, respectively to the right Servlet and to the right virtual host and
 * Restlets (most of the time Application instances).<br>
 * <br>
 * When a request reaches the Servlet container, it is first routed acccording to its web.xml configuration (i.e.
 * declared virtual hosts and webapp context path which is generally the name of the webapp war file). Once the incoming
 * request reaches the ServerServlet and the wrapped Restlet Component, its URI is, for the second time, entirely
 * subject to a separate routing chain. It begins with the virtual hosts, then continue to the URI pattern used when
 * attaching Restlets to the host. The important conclusion is that both routing configurations must be consistent in
 * order to work fine.<br>
 * <br>
 * In deployment mode 3, the context path of the servlet is automatically added. That's what we call the auto-wire
 * feature. This is the default case, and is equivalent to setting the value "true" for the "org.restlet.autoWire"
 * parameter as described above. In modes 1 or 2, if you want to manually control the URI wiring, you can disable the
 * auto-wiring by setting the property to "false".
 * 
...
public class ServerServlet extends HttpServlet {
    ...
    /**
     * The Servlet context initialization parameter's name containing a boolean value. "true" indicates that all
     * applications will be attached to the Component's virtual hosts with the Servlet Context path value.
     */
    ...
    /**
     * The Servlet context initialization parameter's name containing a boolean value. "true" indicates that whether log
     * service will be enable.
     */
    private static final String ENABLE_LOG_KEY = "org.restlet.enableLog";

    /** The default value for the ENABLE_LOG_KEY parameter. */
    private static final String ENABLE_LOG_KEY_DEFAULT = "true";
    ...
    /**
     * The Servlet context initialization parameter's name containing the name of the Servlet context attribute that
     * should be used to store the Restlet Application instance.
     */
    ...
    /**
     * The Servlet context initialization parameter's name containing the name of the Servlet context attribute that
     * should be used to store the Restlet Component instance.
     */
    ...
    /**
     * The Servlet context initialization parameter's name containing the name of the Servlet context attribute that
     * should be used to store the HTTP server connector instance.
     */
        ...
        if (applicationClassName != null) {
            try {
                ...
                try {
                    ...
                    application = (Application) targetClass.getConstructor(Context.class).newInstance(
                            new ServletContextAdapter(this, parentContext));
                }
            } catch (ClassNotFoundException e) {
                log("[Noelios Restlet Engine] - The ServerServlet couldn't find the target class. Please check that your classpath includes "
                        + applicationClassName, e);

            } catch (InstantiationException e) {
                log("[Noelios Restlet Engine] - The ServerServlet couldn't instantiate the target class. Please check this class has an empty constructor "
                        + applicationClassName, e);
            } catch (IllegalAccessException e) {
                log("[Noelios Restlet Engine] - The ServerServlet couldn't instantiate the target class. Please check that you have to proper access rights to "
                        + applicationClassName, e);
            } catch (NoSuchMethodException e) {
                log("[Noelios Restlet Engine] - The ServerServlet couldn't invoke the constructor of the target class. Please check this class has a constructor with a single parameter of Context "
                        + applicationClassName, e);
            } catch (InvocationTargetException e) {
                log("[Noelios Restlet Engine] - The ServerServlet couldn't instantiate the target class. An exception was thrown while creating "
                        + applicationClassName, e);
            }
        }
    ...
    protected Component createComponent() {
        ...
        if (component == null) {
            ...
            final String enableLog = getInitParameter(ENABLE_LOG_KEY, ENABLE_LOG_KEY_DEFAULT);
            if (ENABLE_LOG_KEY_DEFAULT.equalsIgnoreCase(enableLog)) {
    ...
    protected HttpServerHelper createServer(HttpServletRequest request) {
        ...
        if (component != null) {
            ...
            final Server server = new Server(component.getContext().createChildContext(), (List<Protocol>) null,
                    request.getLocalAddr(), request.getLocalPort(), component);
                ...
                if (AUTO_WIRE_KEY_DEFAULT.equalsIgnoreCase(autoWire)) {
                    ...
                    if (addContextPath || addFullServletPath) {
                        ...
                        if (defaultRoute != null) {
                            ...
                            log("[Noelios Restlet Engine] - Attaching restlet: " + defaultRoute.getNext() + " to URI: "
                                    + offsetPath + defaultRoute.getTemplate().getPattern());
                        }

                        // Shift the routes of the default host
                        for (final Route route : component.getDefaultHost().getRoutes()) {
                            log("[Noelios Restlet Engine] - Attaching restlet: " + route.getNext() + " to URI: " + offsetPath
            ...
            synchronized (ServerServlet.class) {
                if (result == null) {
                    ...
                    if (isDefaultComponent()) {
                        ...
                        // application
                        final String applicationAttributeName = getInitParameter(NAME_APPLICATION_ATTRIBUTE,
    ...
    /**
     * Returns the value of a given initialization parameter, first from the Servlet configuration, then from the Web
     * Application context.
     * 
    ...
    /**
     * Indicates if the Component hosted by this Servlet is the default one or one provided by the user.
     * 

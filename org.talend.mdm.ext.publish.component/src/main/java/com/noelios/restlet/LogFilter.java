...
/**
 * Filter logging all calls after their handling by the target Restlet. The current format is similar to IIS 6 logs. The
 * logging is based on the java.util.logging package.
 * 
 ...
 * 
 * @see <a href="http://www.restlet.org/documentation/1.1/tutorial#part07">Tutorial : Filters and call logging</a>
...
public class LogFilter extends Filter {
    ...
    public LogFilter(Context context, LogService logService) {
        ...
        if (logService != null) {
            ...
            if (logService.getLoggerName() != null) {
                ...
                this.logLogger = Logger.getLogger(context.getLogger().getParent().getName() + "."
    ...
    protected void afterHandle(Request request, Response response) {
        ...
        if (this.logTemplate != null) {
            ...
            if (this.logLogger.isInfoEnabled()) {
                ...
                this.logLogger.log(Level.INFO, formatDefault(request, response, duration));
            }
        }
    }
    ...
    protected String formatDefault(Request request, Response response, int duration) {
        ...
        if (this.logService.isIdentityCheck()) {
            final IdentClient ic = new IdentClient(request.getClientInfo().getAddress(), request.getClientInfo().getPort(),
        ...
        final String methodName = (request.getMethod() == null) ? "-" : request.getMethod().getName();
        sb.append((methodName == null) ? "-" : methodName);

        // Append the resource path
        sb.append('\t');
        final String resourcePath = (request.getResourceRef() == null) ? "-" : request.getResourceRef().getPath();
        sb.append((resourcePath == null) ? "-" : resourcePath);

        // Append the resource query
        sb.append('\t');
        final String resourceQuery = (request.getResourceRef() == null) ? "-" : request.getResourceRef().getQuery();
